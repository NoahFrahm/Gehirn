...
class CheckpointerNoah(BaseReporter):
    """
    ...
    def save_checkpoint(self, config, population, species_set, generation):
        ...
        filename = '{0}{1}'.format(self.filename_prefix, generation % 3)
        lst_filename = '{0}{1}'.format(self.filename_prefix, 777)
        print("Saving checkpoint to {0}".format(filename))

        rnd_state = random.getstate()
        with gzip.open(filename, 'w', compresslevel=5) as f:
            data = (generation, config, population, species_set, rnd_state)
            pickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL)
            print(f'saved {generation % 3}')

        with gzip.open(lst_filename, 'w', compresslevel=5) as f:
            data = (generation, config, population, species_set, rnd_state)
            pickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL)
            print('saved 777')
